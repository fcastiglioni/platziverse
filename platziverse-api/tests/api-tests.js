...
test.beforeEach(async () => {
  ...
  // defino lo que seria las funciones utilizano stubs
  ...
  // AgentStub.findByUuid.withArgs(uuidExists).returns(Promise.resolve(agentFixtures.byUuid(uuidExists)))
...
/* test.serial.cb('/api/agent/:uuid', t => {
    request(server)
      .get(`/api/agents/${uuidExists}`)
      .expect(200)
      .expect('Content-Type', /json/)
      .end((error,res) =>{
          t.falsy(error, 'should not return an error')
          const body = JSON.stringify(res.body)
          const expected = JSON.stringify(agentFixtures.byUuid(uuidExists))
